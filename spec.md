...
invite as a temporary invited groupchat connection.
...
connection. Temporary invited groupchat connections are groupchat connections
to the groupchat inviter used by groupchat peers to bootstrap themselves the
...
all current group connections (including temporary invited groupchat
...
the current time plus 300 seconds) to the response, set the `is_stored` number
...
last packet will not have had time to expire (300 second minimum timeout)
before it is resent 120 seconds later.
